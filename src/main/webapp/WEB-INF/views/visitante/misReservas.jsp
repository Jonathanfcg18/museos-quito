...
<div class="container">
  ...
  <div class="filtros-bar">
    <form method="get"
          action="${pageContext.request.contextPath}/reservas/mis-reservas"
          class="form-busqueda">
      <div class="input-group email-search-wrap">
        <span style="display:flex;align-items:center;padding:0 0.75rem;
                     background:var(--bg);border:1px solid var(--border);
                     border-right:none;border-radius:6px 0 0 6px;font-size:1.1rem;">
          ✉️
        </span>
        <input type="email" name="email"
               value="${not empty email ? email :
                        (not empty sessionScope.usuarioSesion ? sessionScope.usuarioSesion.email : '')}"
      ...
      <c:when test="${empty reservas}">
        <div class="empty-state">
          ...
          <a href="${pageContext.request.contextPath}/museos"
             class="btn btn-primary">Ver museos disponibles</a>
        </div>
      </c:when>
      <c:otherwise>
